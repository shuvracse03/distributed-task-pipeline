...
#Get all product links from single page
...
def insert_to_db(json_list):
    ...
    for elem in json_list:
        product_list.append(
            Product(
                title=elem["title"],
                parent_url=elem["parent_url"],
                description=elem["description"],
                upc=elem["upc"],
                product_type=elem["product_type"],
                price_exclude_tax=elem["price_exclude_tax"],
                price_include_tax=elem["price_include_tax"],
                currency=elem["currency"],
                tax=elem["tax"],
                stock=elem["stock"],
                no_reviews=elem["no_reviews"],
            )
        )
    Product.objects.bulk_create(product_list)  # Bulk insert

#Get total number of pages, scrape page by page
def get_product_links(url):
    ...
    try:
        no_pages = int(
            soup.find("li", {"class": "current"}).text.strip().split(" ")[-1]
        )
    except:
        no_pages = 1
    all_links = []
...
#Use bs4 to get information from DOM elements
...
#Merge the book information to a list of jsons
...
#Get all valid book categories from left-side menu
...
#first function to be called the view, it divides the task parallely
@app.task
def scrape_books(user_selection):
    ...
    jobs = group(
        process_link.s(item) for item in chosen_cats
    )  # A group is used to execute several tasks in parallel. 
    jobs.apply_async()
